...
// Sample of RuNda::Tagger class.
int main (int argc, char **argv) {
  char input[] = "太郎は次郎が持っている本を花子に渡した。";

  RuNda::Tagger *tagger = RuNda::createTagger("");
  CHECK(tagger);
  ...
  // Gets Node object.
  const RuNda::Node* node = tagger->parseToNode(input);
  CHECK(node);
  ...
  // Dictionary info.
  const RuNda::DictionaryInfo *d = tagger->dictionary_info();
  for (; d; d = d->next) {
