...
[ -f configure.in ] || {
  echo "autogen.sh: run this command only at the top of a RuNda source tree."
  exit 1
}
...
(autoconf --version) < /dev/null > /dev/null 2>&1 || {
  echo
  echo "You must have autoconf installed to compile RuNda."
...
(automake --version) < /dev/null > /dev/null 2>&1 || {
  echo
  echo "You must have automake installed to compile RuNda."
...
echo "Generating configure script and Makefiles for RuNda."
...
echo "Configuring RuNda."
conf_flags="--enable-maintainer-mode"
echo Running $./configure $conf_flags "$@" ...
./configure $conf_flags "$@"
echo "Now type 'make' to compile RuNda."
