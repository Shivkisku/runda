...
bool EncoderLearnerTagger::read(std::istream *is,
  ...
  size_t pos = 0;
  for (size_t i = 0; corpus[i]->stat != RUNDA_EOS_NODE; ++i) {
    LearnerNode *found = 0;
  ...
  for (size_t i = 0; i < corpus.size(); ++i) {
    ...
    for (LearnerNode *node = begin_node_list_[pos]; node; node = node->bnext) {
      if (corpus[i]->stat == RUNDA_EOS_NODE ||
    ...
    if (lpath->rnode->stat != RUNDA_EOS_NODE) {
