...
bool Viterbi::analyze(Lattice *lattice) const {
  ...
  bool result = false;
  if (lattice->has_request_type(RUNDA_NBEST) ||
      lattice->has_request_type(RUNDA_MARGINAL_PROB)) {
    // IsAllPath=true
...
bool Viterbi::forwardbackward(Lattice *lattice) {
  if (!lattice->has_request_type(RUNDA_MARGINAL_PROB)) {
    return true;
  }
...
bool Viterbi::buildAllLattice(Lattice *lattice) {
  if (!lattice->has_request_type(RUNDA_ALL_MORPHS)) {
    return true;
  }
...
bool Viterbi::buildAlternative(Lattice *lattice) {
  ...
  for (const Node *node = bos_node; node; node = node->next) {
    if (node->stat == RUNDA_BOS_NODE || node->stat == RUNDA_EOS_NODE) {
      continue;
    }
...
bool Viterbi::initPartial(Lattice *lattice) {
  if (!lattice->has_request_type(RUNDA_PARTIAL)) {
    if (lattice->has_constraint()) {
      lattice->set_boundary_constraint(0, RUNDA_TOKEN_BOUNDARY);
      lattice->set_boundary_constraint(lattice->size(),
                                       RUNDA_TOKEN_BOUNDARY);
    }
    return true;
  }
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    lattice->set_boundary_constraint(pos, RUNDA_TOKEN_BOUNDARY);
    lattice->set_boundary_constraint(pos + len, RUNDA_TOKEN_BOUNDARY);
    if (feature) {
