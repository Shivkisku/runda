...
int load_request_type(const Param &param) {
  int request_type = RUNDA_ONE_BEST;

  if (param.get<bool>("allocate-sentence")) {
    request_type |= RUNDA_ALLOCATE_SENTENCE;
  }

  if (param.get<bool>("partial")) {
    request_type |= RUNDA_PARTIAL;
  }

  if (param.get<bool>("all-morphs")) {
    request_type |= RUNDA_ALL_MORPHS;
  }

  if (param.get<bool>("marginal")) {
    request_type |= RUNDA_MARGINAL_PROB;
  }

  const int nbest = param.get<int>("nbest");
  if (nbest >= 2) {
    request_type |= RUNDA_NBEST;
  }
...
bool load_dictionary_resource(Param *param) {
  ...
  if (rcfile.empty()) {
    ...
    if (homedir) {
      const std::string s = RuNda::create_filename(std::string(homedir),
                                                   ".rundarc");
  ...
  if (rcfile.empty()) {
    const char *rcenv = getenv("RUNDARC");
    if (rcenv) {
      rcfile = rcenv;
    }
  }
  ...
  if (rcfile.empty()) {
    ...
    const DWORD len = ::GetEnvironmentVariableW(L"RUNDARC",
                                                buf.get(),
                                                buf.size());
  ...
  if (rcfile.empty()) {
    ::RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"software\\runda", 0, KEY_READ, &hKey);
    qvres = ::RegQueryValueExW(hKey, L"rundarc", 0, &vt,
  ...
  if (rcfile.empty()) {
    ::RegOpenKeyExW(HKEY_CURRENT_USER, L"software\\runda", 0, KEY_READ, &hKey);
    qvres = ::RegQueryValueExW(hKey, L"rundarc", 0, &vt,
                       reinterpret_cast<BYTE *>(v.get()), &size);
    ::RegCloseKey(hKey);
    if (qvres == ERROR_SUCCESS && vt == REG_SZ) {
  ...
  if (rcfile.empty()) {
    ...
    if (vt != 0) {
      ...
      const std::wstring path =
          std::wstring(drive.get()) + std::wstring(dir.get()) + L"rundarc";
