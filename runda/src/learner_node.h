...
#ifndef RUNDA_LEARNER_NODE_H_
#define RUNDA_LEARNER_NODE_H_
...
struct runda_learner_path_t {
  struct runda_learner_node_t*  rnode;
  struct runda_learner_path_t*  rnext;
  struct runda_learner_node_t*  lnode;
  struct runda_learner_path_t*  lnext;
  double                        cost;
  const int                     *fvector;
};

struct runda_learner_node_t {
  struct runda_learner_node_t *prev;
  struct runda_learner_node_t *next;
  struct runda_learner_node_t *enext;
  struct runda_learner_node_t *bnext;
  struct runda_learner_path_t *rpath;
  struct runda_learner_path_t *lpath;
  struct runda_learner_node_t *anext;
  const char                  *surface;
  const char                  *feature;
  unsigned int                 id;
  ...
  double                       cost;
  const int                    *fvector;
  struct runda_token_t         *token;
};

namespace RuNda {

typedef struct runda_learner_path_t LearnerPath;
typedef struct runda_learner_node_t LearnerNode;
...
inline bool node_cmp_eq(const LearnerNode &node1,
  ...
  if (node1.length == node2.length &&
    ...
    // There is NO case when node1 becomes RUNDA_UNK_NODE
    if (node2.stat == RUNDA_UNK_NODE)
...
inline bool is_empty(LearnerPath *path) {
  return ((!path->rnode->rpath && path->rnode->stat != RUNDA_EOS_NODE) ||
          (!path->lnode->lpath && path->lnode->stat != RUNDA_BOS_NODE) );
}
...
inline void calc_expectation(LearnerPath *path, double *expected, double Z) {
  ...
  if (path->rnode->stat != RUNDA_EOS_NODE) {
