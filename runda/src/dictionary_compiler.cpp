...
class DictionaryComplier {
 public:
  static int run(int argc, char **argv) {
    static const RuNda::Option long_options[] = {
      ...
      { "charset",   'c',  RUNDA_DEFAULT_CHARSET, "ENC",
        "make charset of binary dictionary ENC (default "
        RUNDA_DEFAULT_CHARSET ")"  },
      { "charset",   't',  RUNDA_DEFAULT_CHARSET, "ENC", "alias of -c"  },
      { "dictionary-charset",  'f',  RUNDA_DEFAULT_CHARSET,
...
#define OCONF(file) create_filename(outdir, std::string(file)).c_str()
    ...
    if (!userdic.empty()) {
      ...
      param.set("type", static_cast<int>(RUNDA_USR_DIC));
      ...
      if (opt_unknown) {
        ...
        param.set("type", static_cast<int>(RUNDA_UNK_DIC));
      ...
      if (opt_sysdic) {
        CHECK_DIE(dic.size()) << "no dictionaries are specified";
        param.set("type", static_cast<int>(RUNDA_SYS_DIC));
...
int runda_dict_index(int argc, char **argv) {
  return RuNda::DictionaryComplier::run(argc, argv);
}
