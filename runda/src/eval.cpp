...
class Eval {
 ...
 public:
  static bool eval(int argc, char **argv) {
    ...
    RuNda::ostream_wrapper ofs(output.c_str());
 ...
 public:
  static int run(int argc, char **argv) {
    ...
    for (size_t i = 0; i < files.size(); ++i) {
      RuNda::istream_wrapper ifs(files[i].c_str());
...
// exports
int runda_system_eval(int argc, char **argv) {
  return RuNda::Eval::eval(argc, argv);
}

int runda_test_gen(int argc, char **argv) {
  return RuNda::TestSentenceGenerator::run(argc, argv);
}
