...
const RuNda::Option long_options[] = {
  ...
  { "dump-config", 'P', 0, 0, "dump RuNda parameters" },
...
ModelImpl::ModelImpl()
    : viterbi_(new Viterbi), writer_(new Writer),
      request_type_(RUNDA_ONE_BEST), theta_(0.0) {}
...
TaggerImpl::TaggerImpl()
    : current_model_(0),
      request_type_(RUNDA_ONE_BEST), theta_(kDefaultTheta) {}
...
bool TaggerImpl::partial() const {
  return request_type_ & RUNDA_PARTIAL;
}
...
void TaggerImpl::set_lattice_level(int level) {
  switch (level) {
    case 0: request_type_ |= RUNDA_ONE_BEST;
      break;
    case 1: request_type_ |= RUNDA_NBEST;
      break;
    case 2: request_type_ |= RUNDA_MARGINAL_PROB;
      break;
    default:
      break;
  }
...
int TaggerImpl::lattice_level() const {
  if (request_type_ & RUNDA_MARGINAL_PROB) {
    return 2;
  } else if (request_type_ & RUNDA_NBEST) {
    return 1;
  } else {
    return 0;
  }
...
void TaggerImpl::set_all_morphs(bool all_morphs) {
  if (all_morphs) {
    request_type_ |= RUNDA_ALL_MORPHS;
  } else {
    request_type_ &= ~RUNDA_ALL_MORPHS;
  }
}
...
bool TaggerImpl::all_morphs() const {
  return request_type_ & RUNDA_ALL_MORPHS;
}
...
bool TaggerImpl::parseNBestInit(const char *str, size_t len) {
  Lattice *lattice = mutable_lattice();
  initRequestType();
  lattice->add_request_type(RUNDA_NBEST);
  lattice->set_sentence(str, len);
  if (!parse(lattice)) {
    set_what(lattice->what());
    return false;
...
void LatticeImpl::set_sentence(const char *sentence, size_t len) {
  ...
  if (has_request_type(RUNDA_ALLOCATE_SENTENCE) ||
      has_request_type(RUNDA_PARTIAL)) {
...
bool LatticeImpl::next() {
  if (!has_request_type(RUNDA_NBEST)) {
    set_what("RUNDA_NBEST request type is not set");
    return false;
  }
...
void LatticeImpl::set_boundary_constraint(size_t pos,
                                          int boundary_constraint_type) {
  if (boundary_constraint_.empty()) {
    boundary_constraint_.resize(size() + 4, RUNDA_ANY_BOUNDARY);
  }
...
void LatticeImpl::set_feature_constraint(size_t begin_pos, size_t end_pos,
  ...
  set_boundary_constraint(begin_pos, RUNDA_TOKEN_BOUNDARY);
  set_boundary_constraint(end_pos, RUNDA_TOKEN_BOUNDARY);
  for (size_t i = begin_pos + 1; i < end_pos; ++i) {
    set_boundary_constraint(i, RUNDA_INSIDE_TOKEN);
  }
...
int runda_do(int argc, char **argv) {
#define WHAT_ERROR(msg) do {                    \
  ...
  RuNda::Param param;
  if (!param.open(argc, argv, RuNda::long_options)) {
  ...
  RuNda::scoped_ptr<RuNda::ModelImpl> model(new RuNda::ModelImpl);
  if (!model->open(param)) {
    std::cout << RuNda::getLastError() << std::endl;
    return EXIT_FAILURE;
  }
  ...
  RuNda::ostream_wrapper ofs(ofilename.c_str());
  if (!*ofs) {
  ...
  if (param.get<bool>("dictionary-info")) {
    for (const RuNda::DictionaryInfo *d = model->dictionary_info();
         d; d = d->next) {
  ...
  RuNda::scoped_array<char> ibuf_data(new char[ibufsize]);
  char *ibuf = ibuf_data.get();

  RuNda::scoped_ptr<RuNda::Tagger> tagger(model->createTagger());
  ...
  for (size_t i = 0; i < rest.size(); ++i) {
    RuNda::istream_wrapper ifs(rest[i].c_str());
    if (!*ifs) {
    ...
    while (true) {
      if (!partial) {
        ...
        std::string sentence;
        RuNda::scoped_fixed_array<char, BUF_SIZE> line;
        for (;;) {
