...
bool is_valid_node(const Lattice *lattice,  N *node) {
  ...
  if (lattice->boundary_constraint(end_pos) == RUNDA_INSIDE_TOKEN) {
    return false;
  }
  ...
  if (lattice->boundary_constraint(begin_pos) == RUNDA_TOKEN_BOUNDARY &&
      lattice->boundary_constraint(end_pos) == RUNDA_TOKEN_BOUNDARY &&
...
N *Tokenizer<N, P>::lookup(const char *begin, const char *end,
  ...
  if (isPartial) {
    ...
    for (size_t n = begin_pos + 1; n < lattice->size(); ++n) {
      if (lattice->boundary_constraint(n) == RUNDA_TOKEN_BOUNDARY) {
